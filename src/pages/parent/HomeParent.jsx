    ...
    return (
        <Navbar>
            <Container sx={{marginTop:"120px"}}>
                <Typography sx={{color:"#000000",fontWeight:"700",fontSize:"24px",marginBottom:"8px"}}>{t('parent_welcome')}</Typography>
                <Typography sx={{color:"#000000",fontWeight:"500",fontSize:"18px",marginBottom:"32px"}}>{t('parent_desc')}</Typography> 
                    ...
                    <Grid container spacing={2}>
                        {
                          data?.data.map(st =>{
                            ...
                            <Paper sx={{padding:"8px 6px",display:"flex",flexDirection:"column",alignItems:"center"}}>
                                ...
                                <Typography sx={{fontWeight:"500",marginY:"10px",fontSize:"18px",minHeight:"50px",textAlign:"center"}}>{t('student')} : {st.name}</Typography>
                                <Button onClick={()=>navigate(`/parent-dash/student/${st.id}`)}>{t('view')}</Button>
                            </Paper>
                        </Grid>
                          })
                        }
