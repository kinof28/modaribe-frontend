...
export default function ContactPersonTeacher({item , selectChat,lastMessage,active}) {
    ...
    return (
            ...
            cursor:"pointer" , backgroundColor:active?"#eee":"" , padding:"12px 8px" , borderRadius:"6px"}} onClick={selectChat}>
                <Avatar alt={data?.data?.name} src={`${process.env.REACT_APP_API_KEY}images/${data?.data?.image}`} sx={{width:"45px",height:"45px"}}/>
                <Box>
                    <Typography sx={{margin:0}}>{data?.data?.name}</Typography>
