...
export default function UpdateStudyLevel({handleClose,level,setLevels}) {
    const {token} = useSelector((state)=>state.admin)
    const {t} = useTranslation()
    const { register,control, formState: { errors }, handleSubmit } = useForm({
        defaultValues: {
            title_ar:level?.titleAR,
            title_en:level?.titleEN
        }
    });
    ...
    {
        try{
            const response = await fetch(`${process.env.REACT_APP_API_KEY}api/v1/admin/updateLevel/${level.id}`,{
                method:"PUT",
                headers:{
                    "Authorization":token,
            ...
            setLevels(back=>back.map(item=>
                {
                    return item.id===level.id?{...item,titleAR:data.title_ar,titleEN:data.title_en}:item
                }))
            handleClose()
        }
        catch(err)
