...
export default function UpdateCurriculms({handleClose,curriculm,setCurriculums}) {
    const {t} = useTranslation()
    const { register,control, formState: { errors }, handleSubmit } = useForm({
        defaultValues: {
            title_ar:curriculm?.titleAR,
            title_en:curriculm?.titleEN
        }
    });
    ...
    {
        try{
            const response = await fetch(`${process.env.REACT_APP_API_KEY}api/v1/admin/updateCurriculum/${curriculm.id}`,{
                method:"PUT",
                headers:{
                    "Authorization":token,
            ...
            setCurriculums(back=>back.map(item=>
                {
                    return item.id===curriculm.id?{...item,titleAR:data.title_ar,titleEN:data.title_en}:item
                }))
            handleClose()
        }
        catch(err)
